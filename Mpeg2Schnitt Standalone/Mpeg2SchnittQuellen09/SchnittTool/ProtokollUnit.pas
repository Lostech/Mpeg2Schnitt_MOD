...
USES SysUtils, Classes, StdCtrls,    // Standardunits
...
PROCEDURE Protokoll_schreiben(Text: STRING; Level: Integer = 0);

PROCEDURE ProtokollText_schreiben(TextListe: TStrings; Level: Integer = 0);

VAR Protokollfenster : TMemo;
    ProtokollLevel : Integer = 2;            // 0 - alles anzeigen
                                             // 1 - Fehler anzeigen
                                             // 2 - Status anzeigen
                                             // 3 - Infos anzeigen
                                             // 4 - interne Infos anzeigen
    Datei_schliessen : Boolean = False;
...
BEGIN
  ...
  ELSE
  BEGIN
    Protokoll_schreiben(WortProtokollende, -1);  
...
PROCEDURE Protokoll_schreiben(Text: STRING; Level: Integer = 0);
BEGIN
  IF (Level <= ProtokollLevel) OR (ProtokollLevel = 0) THEN
  BEGIN
    Text := DateTimeToStr(Now) + '  :  ' + Text;
    Protokollfenster.Lines.Add(Text);
    IF ArbeitsumgebungObj.Protokollerstellen OR
       (Level = -1) THEN
    BEGIN
      IF DateiHandle < 0 THEN
      BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        ELSE
          Text := StringOfChar(' ', Length(DateTimeToStr(Now)));
        Text := Text + TextListe.Strings[I] + Chr(13) + Chr(10);
        FileWrite(DateiHandle, Text[1], Length(Text));
      END;
      IF Datei_schliessen THEN
        Protokoll_schliessen;
