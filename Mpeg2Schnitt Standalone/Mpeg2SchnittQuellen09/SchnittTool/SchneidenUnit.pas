...
USES Classes, ComCtrls, SysUtils, Forms, StdCtrls, Grids, Windows,
     ...
     Sprachen, AllgVariablen, VideoschnittUnit, AudioschnittUnit;
...
FUNCTION Speicherplatzpruefen(Dateiname: STRING; SchnittListe: TStrings; Audio: Boolean = False): Integer;
...
BEGIN
  ...
  FOR I := 0 TO SchnittListe.Count - 1 DO
    IF Audio THEN
      Groesse := Groesse + TSchnittpunkt(SchnittListe.Objects[I]).AudioGroesse
    ELSE
      Groesse := Groesse + TSchnittpunkt(SchnittListe.Objects[I]).VideoGroesse;
  Laufwerk := Diskfree(Byte(Ord(Uppercase(Dateiname)[1])-64));
  IF Laufwerk > -1 THEN
  BEGIN
    IF Laufwerk < Groesse THEN
      Result := -1
    ELSE
    BEGIN
      I := LaufwerksInfo(Dateiname[1]);
      IF (I > -1) AND (I < 3) AND                                               // FAT oder FAT32
         (Groesse > 4294967295) THEN                                            // 4 GByte -1
        Result := -2
      ELSE
        Result := 0;
    END;
...
BEGIN
  Result := Speicherplatzpruefen(Dateiname, SchnittListe);
  IF Result > -1 THEN
  BEGIN
...
BEGIN
  Result := Speicherplatzpruefen(Dateiname, SchnittListe, True);
  IF Result > -1 THEN
  BEGIN
...
BEGIN
  ...
  TRY
    ...
    IF Result > - 1 THEN
    BEGIN
      IF Pos(UpperCase(ExtractFileExt(Dateiname)), UpperCase(ArbeitsumgebungObj.DateiendungenVideo)) > 0 THEN  // Videoendung entfernen
        Dateiname := ChangeFileExt(Dateiname, '');
      Result := ListeSchneidenAudio(Dateiname, SchnittListe, AusgabeListe, ProjektInfoObj);
      IF Result > -1 THEN
      BEGIN
        IF Pos(UpperCase(ExtractFileExt(Dateiname)), UpperCase(ArbeitsumgebungObj.DateiendungenAudio)) > 0 THEN  // Audioendung entfernen
          Dateiname := ChangeFileExt(Dateiname, '');
