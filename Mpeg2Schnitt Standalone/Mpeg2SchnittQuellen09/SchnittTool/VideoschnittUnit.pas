...
//      MpegHeader.FortschrittsEndwert := FortschrittsEndwert;
      MpegHeader.Dateioeffnen(Dateiname);
...
BEGIN
  ...
  BEGIN
    ...
    Variablesetzen(Variablen, '$FrameHight#', IntToStr(Sequenzheader.BildHoehe));
...
//  END;
  ...
  Framerate := StrToFloatDef(VariablenersetzenText('$Framerate#', Variablen), -1);
    ...
    BEGIN
      ...
      BEGIN
        ...
        TRY
          ...
          Variablen.Add(ErsetzeZeichen(FZwischenverzeichnis + 'PartFile-' + ExtractFileName(ChangeFileExt(Quelldateiname, '')) + '-' + IntToStr(AnfangsIndex) + '-' + IntToStr(EndIndex), ['&', ',']) + ExtractFileExt(Quelldateiname));
          Variablen.Add('$PartFiled2v#');
          Variablen.Add(ChangeFileExt(Variablen.Strings[Variablen.Count - 2], '.d2v'));
          Variablen.Add('$NewFile#');
          Variablen.Add(ErsetzeZeichen(FZwischenverzeichnis + 'NewFile-$Scriptname#-' + ExtractFileName(ChangeFileExt(Quelldateiname, '')) + '-' + IntToStr(AnfangsIndex) + '-' + IntToStr(EndIndex), ['&', ','])  + ExtractFileExt(Quelldateiname));
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      IF Assigned(FFortschrittsEndwert) THEN
        FFortschrittsEndwert^ := FFortschrittsEndwert^ + Schnittpunkt.VideoGroesse;
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            Result := Quelldateioeffnen(Schnittpunkt.VideoName, Schnittpunkt.VideoListe, Schnittpunkt.VideoIndexListe);
            ...
            BEGIN
              ...
              BEGIN
                HInt1 := Schnittpunkt.Anfang + Round(FAspectratioOffset / BildlaengeausSeqHeaderFramerate(Sequenzheader.Framerate));
                IF HInt1 > Schnittpunkt.Ende THEN
                  HInt1 := Schnittpunkt.Ende;
                OffsetAspectratio := OffsetAspectratiolesen(HInt1);
              END;
              EffektEintrag := Schnittpunkt.VideoEffekt;
              ...
              BEGIN                                               // zwei Einzeleffekte
                IF Assigned(EffektEintrag) AND
                  ...
                  Anfang := NaechstesBild(1, Round(EffektEintrag.AnfangLaenge * Schnittpunkt.Framerate / 1000) + Schnittpunkt.Anfang, IndexListe)
                ELSE
                  IF FFramegenauschneiden AND                     // framegenau
                     ((Schnittpunkt.Anfangberechnen AND $80) = $00) THEN  // berechnen
                  BEGIN
                    Anfang := NaechstesBild(1, Schnittpunkt.Anfang, IndexListe);
                    IF (Anfang - Schnittpunkt.Anfang > 0) AND (Anfang - Schnittpunkt.Anfang < FminAnfang) THEN
                      Anfang := NaechstesBild(1, Schnittpunkt.Anfang + FminAnfang, IndexListe);
                  END
                  ELSE
...
//                HAnfang := Schnittpunkt.Anfang;
                ...
                BEGIN
                  ...
                  IF ((Schnittpunkt.VideoEffekt.EndeLaenge = 0) OR (Schnittpunkt.VideoEffekt.EndeEffektName = '')) AND       // kein Effekt am Ende des Schnittpunkts 1
                     (((Schnittpunkt2.VideoEffekt.AnfangLaenge = 0) AND (Schnittpunkt2.VideoEffekt.EndeLaenge > 0)) OR (Schnittpunkt2.VideoEffekt.AnfangEffektName = '')) AND // kein Effekt am Anfang des Schnittpunkts 2 und kein Effekt über den gesamten Schnitt 2
                     (Schnittpunkt.VideoName = Schnittpunkt2.VideoName) AND
                ...
                IF Assigned(EffektEintrag) AND
                  ...
                  Ende := VorherigesBild(2, Schnittpunkt.Ende - Round(EffektEintrag.EndeLaenge * Schnittpunkt.Framerate / 1000), IndexListe) // Endeeffekt
                ELSE
                  IF FFramegenauschneiden AND                     // framegenau
                     ((Schnittpunkt.Endeberechnen AND $80) = $00) THEN  // berechnen
                  BEGIN
                    Ende := VorherigesBild(2, Schnittpunkt.Ende, IndexListe);
                    IF (Schnittpunkt.Ende - Ende > 0) AND (Schnittpunkt.Ende - Ende < FminEnde) THEN
                      Ende := VorherigesBild(2, Schnittpunkt.Ende - FminEnde, IndexListe);
                  END
                  ELSE
                  BEGIN                                           // nicht framegenau
                  ...
                  END;
                IF Anfang > Schnittpunkt.Ende + 1 THEN
                  Anfang := Schnittpunkt.Ende + 1;                // Der Anfangseffekt darf nicht länger als der Schnitt sein
                IF Anfang > Ende THEN
                  Ende := Anfang - 1;                             // Der Endeeffekt darf nicht vor dem Ende des Anfangseffektes beginnen
                IF Assigned(EffektEintrag) AND
                   (EffektEintrag.AnfangLaenge > 0) AND (EffektEintrag.AnfangEffektName <> '') THEN
                BEGIN                                             // Anfangseffekt
                  Result := Effektberechnen(Schnittpunkt.Anfang, Anfang - 1,
                                            EffektEintrag.AnfangEffektDateiname, EffektEintrag.AnfangEffektName,
                                            ':eff', EffektEintrag.AnfangEffektParameter,
                                            Round(EffektEintrag.AnfangLaenge * Schnittpunkt.Framerate / 1000),
                                            Sequenzheader, 'In');
                  IF Result < 0 THEN
                    Result := Result - 200;
                END
                ...
                BEGIN
                  REPEAT
                    ...
                    Result := KopiereVideoteil(Anfang, HInt1, Schnittpunkt.Framerate, Dateistream, Speicherstream, NeueListe, True, True, True, Bildzaehler);
                    IF Ende < HInt2 THEN
                      HInt2 := Ende;
//                  Result := KopiereVideoteil(Anfang, Ende, Schnittpunkt.Framerate, Dateistream, Speicherstream, NeueListe, ersterSequenzHeader, BildZaehler, True, True);
                    IF Result < 0 THEN
                      Result := Result - 400;
                  ...
                  IF Result > -1 THEN
                    IF Assigned(EffektEintrag) AND
                       (EffektEintrag.EndeLaenge > 0) AND (EffektEintrag.EndeEffektName <> '') THEN
                    BEGIN                                         // Endeeffekt
                      Result := Effektberechnen(Ende + 1, Schnittpunkt.Ende,
                                                EffektEintrag.EndeEffektDateiname, EffektEintrag.EndeEffektName,
                                                ':eff', EffektEintrag.EndeEffektParameter,
                                                Trunc(EffektEintrag.EndeLaenge * Schnittpunkt.Framerate / 1000),
                                                Sequenzheader, 'Out');
                      IF Result < 0 THEN
                        Result := Result - 500;
                    END
                  ...
                  BEGIN
                    ...
                    BEGIN
                      ...
                      BEGIN
                        ...
                        IF Result = 0 THEN
                        BEGIN
                          IF FD2VDateierstellen AND Assigned(Headerklein) THEN
                          BEGIN
...
BEGIN
  ...
  BEGIN
    ...
    IF Assigned(Liste) AND
      (FIndexDateierstellen OR FD2VDateierstellen) THEN    // Wenn eine neue Indexdatei erstellt wurde,
    BEGIN
      Headerklein := THeaderklein.Create;                  // neuen Listenpunkt erzeugen,
