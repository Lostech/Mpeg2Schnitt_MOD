...
uses
  Windows,
  SysUtils;           // für FileExists

var
  DDrawDLL : HMODULE = 0;
  DirektDraw_OK : Boolean;
 ...
 *)
  IDirectDrawSurface4 = interface (IUnknown)
    ...
    function BltBatch(const lpDDBltBatch : TDDBltBatch; dwCount : LongWord; dwFlags : LongWord) : HResult; stdcall;
    function BltFast(dwX : LongWord; dwY : LongWord; lpDDSrcSurface : IDirectDrawSurface4; lpSrcRect : PRect; dwTrans : LongWord) : HResult; stdcall;
    ...
    function EnumOverlayZOrders(dwFlags : LongWord; lpContext : Pointer; lpfnCallback : TDDEnumSurfacesCallback2) : HResult; stdcall;
    ...
    function IsLost : HResult; stdcall;
    function Lock(lpDestRect : PRect; out lpDDSurfaceDesc : TDDSurfaceDesc2; dwFlags : LongWord; hEvent : THandle) : HResult; stdcall;
    function ReleaseDC(hDC : Windows.HDC) : HResult; stdcall;
    function _Restore : HResult; stdcall;
    function SetClipper(lpDDClipper : IDirectDrawClipper) : HResult; stdcall;
    function SetColorKey(dwFlags : LongWord; lpDDColorKey : PDDColorKey) : HResult; stdcall;
    ...
    function UpdateOverlay(lpSrcRect : PRect; lpDDDestSurface : IDirectDrawSurface4; lpDestRect : PRect; dwFlags : LongWord; lpDDOverlayFx : PDDOverlayFX) : HResult; stdcall;
    function UpdateOverlayDisplay(dwFlags : LongWord) : HResult; stdcall;
   ...
   (*** Added in the v2 interface ***)
    ...
    function PageUnlock(dwFlags : LongWord) : HResult; stdcall;
   (*** Added in the V3 interface ***)
    function SetSurfaceDesc(const lpddsd2 : TDDSurfaceDesc2; dwFlags : LongWord) : HResult; stdcall;
   (*** Added in the v4 interface ***)
    function SetPrivateData(const guidTag : TGUID; lpData : Pointer; cbSize : LongWord; dwFlags : LongWord) : HResult; stdcall;
  ...
  IDirectDrawSurface7 = interface (IUnknown)
    ...
    function AddOverlayDirtyRect (const lpRect : TRect) : HResult; stdcall;
    function Blt (lpDestRect : PRect; lpDDSrcSurface : IDirectDrawSurface7; lpSrcRect : PRect; dwFlags : LongWord; lpDDBltFx : PDDBltFX) : HResult; stdcall;
    function BltBatch (const lpDDBltBatch : TDDBltBatch; dwCount : LongWord; dwFlags : LongWord) : HResult; stdcall;
    function BltFast (dwX : LongWord; dwY : LongWord; lpDDSrcSurface : IDirectDrawSurface7; lpSrcRect : PRect; dwTrans : LongWord) : HResult; stdcall;
    ...
    function EnumOverlayZOrders (dwFlags : LongWord; lpContext : Pointer; lpfnCallback : TDDEnumSurfacesCallback7) : HResult; stdcall;
    function Flip (lpDDSurfaceTargetOverride : IDirectDrawSurface7; dwFlags : LongWord) : HResult; stdcall;
    function GetAttachedSurface (const lpDDSCaps : TDDSCaps2; out lplpDDAttachedSurface : IDirectDrawSurface7) : HResult; stdcall;
    function GetBltStatus(dwFlags : LongWord) : HResult; stdcall;
    function GetCaps(out lpDDSCaps : TDDSCaps2) : HResult; stdcall;
    function GetClipper(out lplpDDClipper : IDirectDrawClipper) : HResult; stdcall;
    function GetColorKey(dwFlags : LongWord; out lpDDColorKey : TDDColorKey) : HResult; stdcall;
    ...
    function SetPalette(lpDDPalette : IDirectDrawPalette) : HResult; stdcall;
    function Unlock(lpRect : PRect) : HResult; stdcall;
    function UpdateOverlay(lpSrcRect : PRect; lpDDDestSurface : IDirectDrawSurface7; lpDestRect : PRect; dwFlags : LongWord; lpDDOverlayFx : PDDOverlayFX) : HResult; stdcall;
    function UpdateOverlayDisplay(dwFlags : LongWord) : HResult; stdcall;
 ...
 *)
  IDirectDrawGammaControl = interface (IUnknown)
    ...
    function GetGammaRamp(dwFlags : LongWord; out lpRampData : TDDGammaRamp) : HResult; stdcall;
    function SetGammaRamp(dwFlags : LongWord; const lpRampData : TDDGammaRamp) : HResult; stdcall;
  end;
...
var
  ...
  DirectDrawEnumerateExA : function(lpCallback : TDDEnumCallbackExA; lpContext : Pointer; dwFlags : LongWord) : HResult; stdcall;
  DirectDrawEnumerateExW : function(lpCallback : TDDEnumCallbackExW; lpContext : Pointer; dwFlags : LongWord) : HResult; stdcall;
  DirectDrawEnumerateEx  : function(lpCallback : TDDEnumCallbackEx; lpContext : Pointer; dwFlags : LongWord) : HResult; stdcall;
  ...
  DirectDrawCreateClipper : function(dwFlags : LongWord; out lplpDDClipper : IDirectDrawClipper; pUnkOuter : IUnknown) : HResult; stdcall;
 ...
 *)
function GET_WHQL_YEAR(dwWHQLLevel: LongWord) : LongWord;
function GET_WHQL_MONTH(dwWHQLLevel: LongWord) : LongWord;
function GET_WHQL_DAY(dwWHQLLevel: LongWord) : LongWord;
 ...
 *)
  ...
  TDDVideoPortConnect = packed record
    dwSize      : LongWord;  // size of the TDDVideoPortConnect structure
    dwPortWidth : LongWord;  // Width of the video port
    guidTypeID  : TGUID;     // Description of video port connection
    dwFlags     : LongWord;  // Connection flags
    dwReserved1 : LongWord;  // Reserved, set to zero.
  end;
 ...
 *)
  ...
  TDDVideoPortCaps = packed record
    dwSize                        : LongWord;  // size of the TDDVideoPortCaps structure
    dwFlags                       : LongWord;  // indicates which fields contain data
    dwMaxWidth                    : LongWord;  // max width of the video port field
    dwMaxVBIWidth                 : LongWord;  // max width of the VBI data
    dwMaxHeight                   : LongWord;  // max height of the video port field
    dwVideoPortID                 : LongWord;  // Video port ID (0 - (dwMaxVideoPorts -1))
    dwCaps                        : LongWord;  // Video port capabilities
    dwFX                          : LongWord;  // More video port capabilities
    dwNumAutoFlipSurfaces         : LongWord;  // Number of autoflippable surfaces
    dwAlignVideoPortBoundary      : LongWord;  // Byte restriction of placement within the surface
    dwAlignVideoPortPrescaleWidth : LongWord;  // Byte restriction of width after prescaling
    dwAlignVideoPortCropBoundary  : LongWord;  // Byte restriction of left cropping
    dwAlignVideoPortCropWidth     : LongWord;  // Byte restriction of cropping width
    dwPreshrinkXStep              : LongWord;  // Width can be shrunk in steps of 1/x
    dwPreshrinkYStep              : LongWord;  // Height can be shrunk in steps of 1/x
    dwNumVBIAutoFlipSurfaces      : LongWord;  // Number of VBI autoflippable surfaces
    dwNumPreferredAutoflip        : LongWord;	 // Optimal number of autoflippable surfaces for hardware
 ...
 *)
  ...
  TDDVideoPortDesc = packed record                 
    dwSize                 : LongWord;             // size of the TDDVideoPortDesc structure
    dwFieldWidth           : LongWord;             // width of the video port field
    dwVBIWidth             : LongWord;             // width of the VBI data
    dwFieldHeight          : LongWord;             // height of the video port field
    dwMicrosecondsPerField : LongWord;             // Microseconds per video field
    dwMaxPixelsPerSecond   : LongWord;             // Maximum pixel rate per second
    dwVideoPortID          : LongWord;             // Video port ID (0 - (dwMaxVideoPorts -1))
    dwReserved1            : LongWord;             // Reserved for future use - set to zero
    VideoPortType          : TDDVideoPortConnect;  // Description of video port connection
    dwReserved2            : LongWord;             // Reserved for future use - set to zero
    dwReserved3            : LongWord;             // Reserved for future use - set to zero
  end;
 ...
 *)
  ...
  TDDVideoPortInfo = packed record
    dwSize                : LongWord;        // Size of the structure
    dwOriginX             : LongWord;        // Placement of the video data within the surface.
    dwOriginY             : LongWord;        // Placement of the video data within the surface.
    dwVPFlags             : LongWord;        // Video port options
    rCrop                 : TRect;           // Cropping rectangle (optional).
    dwPrescaleWidth       : LongWord;        // Determines pre-scaling/zooming in the X direction (optional).
    dwPrescaleHeight      : LongWord;        // Determines pre-scaling/zooming in the Y direction (optional).
    ...
    dwVBIHeight           : LongWord;        // Specifies the number of lines of data within the vertical blanking interval.
    dwReserved1           : LongWord;        // Reserved for future use - set to zero
    dwReserved2           : LongWord;        // Reserved for future use - set to zero
  end;
 ...
 *)
  ...
  TDDVideoPortBandWidth = packed record
    dwSize               : LongWord;   // Size of the structure
    dwCaps               : LongWord;
    dwOverlay            : LongWord;   // Zoom factor at which overlay is supported
    dwColorkey           : LongWord;   // Zoom factor at which overlay w/ colorkey is supported
    dwYInterpolate       : LongWord;   // Zoom factor at which overlay w/ Y interpolation is supported
    dwYInterpAndColorkey : LongWord;   // Zoom factor at which ovelray w/ Y interpolation and colorkeying is supported
    dwReserved1          : LongWord;   // Reserved for future use - set to zero
    dwReserved2          : LongWord;   // Reserved for future use - set to zero
  end;
 ...
 *)
  ...
  TDDVideoPortStatus = record
    ...
    dwFlags       : LongWord;             // Currently not used
    dwReserved1   : LongWord;             // Reserved for future use
    VideoPortType : TDDVideoPortConnect;  // Information about the connection
    dwReserved2   : LongWord;             // Reserved for future use
    dwReserved3   : LongWord;             // Reserved for future use
  end;
 ...
 *)
  ...
  TDDEnumVideoCallback = function(lpTDDVideoPortCaps : PDDVideoPortCaps; lpContext: Pointer) : HResult; stdcall;
 ...
 *)
  IDirectDrawVideoPort = interface (IUnknown)
    ...
    function Flip(lpDDSurface : IDirectDrawSurface; dwFlags : LongWord) : HResult; stdcall;
    function GetBandwidthInfo(var lpddpfFormat : TDDPixelFormat; dwWidth : LongWord; dwHeight : LongWord; dwFlags : LongWord; var lpBandwidth : TDDVideoPortBandWidth) : HResult; stdcall;
    function GetColorControls(var lpColorControl : TDDColorControl) : HResult; stdcall;
    function GetInputFormats(var lpNumFormats : LongWord; var lpFormats : TDDPixelFormat; dwFlags : LongWord) : HResult; stdcall;
    function GetOutputFormats(var lpInputFormat : TDDPixelFormat; var lpNumFormats : LongWord; lpFormats : PDDPixelFormat; dwFlags : LongWord) : HResult; stdcall;
    function GetFieldPolarity(var lpbVideoField : BOOL) : HResult; stdcall;
    function GetVideoLine(var lpdwLine : LongWord) : HResult; stdcall;
    function GetVideoSignalStatus(varlpdwStatus : LongWord) : HResult; stdcall;
    function SetColorControls(var lpColorControl : TDDColorControl) : HResult; stdcall;
    function SetTargetSurface(lpDDSurface : IDirectDrawSurface; dwFlags : LongWord) : HResult; stdcall;
    ...
    function WaitForSync(dwFlags : LongWord; dwLine : LongWord; dwTimeout : LongWord) : HResult; stdcall;
  end;
 ...
 *)
  IDDVideoPortContainer = interface (IUnknown)
    ...
    function CreateVideoPort(dwFlags : LongWord; var lpTDDVideoPortDesc: TDDVideoPortDesc; var lplpDDVideoPort : IDirectDrawVideoPort; pUnkOuter : IUnknown) : HResult; stdcall;
    function EnumVideoPorts(dwFlags : LongWord; lpTDDVideoPortCaps : PDDVideoPortCaps; lpContext : Pointer; lpEnumVideoCallback : TDDEnumVideoCallback) : HResult; stdcall;
    function GetVideoPortConnectInfo(dwPortId : LongWord; var lpNumEntries : LongWord; lpConnectInfo : PDDVideoPortConnect) : HResult; stdcall;
    function QueryVideoPortStatus(dwPortId : LongWord; var lpVPStatus : TDDVideoPortStatus) : HResult; stdcall;
  end;
...
begin
  case Value of
    HResult(DD_OK)                              : Result := 'The request completed successfully.';
    HResult(DDERR_ALREADYINITIALIZED)           : Result := 'This object is already initialized.';
    HResult(DDERR_BLTFASTCANTCLIP)              : Result := ' if a clipper object is attached to the source surface passed into a BltFast call.';
    HResult(DDERR_CANNOTATTACHSURFACE)          : Result := 'This surface can not be attached to the requested surface.';
    HResult(DDERR_CANNOTDETACHSURFACE)          : Result := 'This surface can not be detached from the requested surface.';
    HResult(DDERR_CANTCREATEDC)                 : Result := 'Windows can not create any more DCs.';
    HResult(DDERR_CANTDUPLICATE)                : Result := 'Cannot duplicate primary & 3D surfaces, or surfaces that are implicitly created.';
    HResult(DDERR_CLIPPERISUSINGHWND)           : Result := 'An attempt was made to set a cliplist for a clipper object that is already monitoring an hwnd.';
    HResult(DDERR_COLORKEYNOTSET)               : Result := 'No src color key specified for this operation.';
    HResult(DDERR_CURRENTLYNOTAVAIL)            : Result := 'Support is currently not available.';
    HResult(DDERR_DIRECTDRAWALREADYCREATED)     : Result := 'A DirectDraw object representing this driver has already been created for this process.';
    HResult(DDERR_EXCEPTION)                    : Result := 'An exception was encountered while performing the requested operation.';
    HResult(DDERR_EXCLUSIVEMODEALREADYSET)      : Result := 'An attempt was made to set the cooperative level when it was already set to exclusive.';
    HResult(DDERR_GENERIC)                      : Result := 'Generic failure.';
    HResult(DDERR_HEIGHTALIGN)                  : Result := 'Height of rectangle provided is not a multiple of reqd alignment.';
    HResult(DDERR_HWNDALREADYSET)               : Result := 'The CooperativeLevel HWND has already been set. It can not be reset while the process has surfaces or palettes created.';
    HResult(DDERR_HWNDSUBCLASSED)               : Result := 'HWND used by DirectDraw CooperativeLevel has been subclassed, this prevents DirectDraw from restoring state.';
    HResult(DDERR_IMPLICITLYCREATED)            : Result := 'This surface can not be restored because it is an implicitly created surface.';
    HResult(DDERR_INCOMPATIBLEPRIMARY)          : Result := 'Unable to match primary surface creation request with existing primary surface.';
    HResult(DDERR_INVALIDCAPS)                  : Result := 'One or more of the caps bits passed to the callback are incorrect.';
    HResult(DDERR_INVALIDCLIPLIST)              : Result := 'DirectDraw does not support the provided cliplist.';
    HResult(DDERR_INVALIDDIRECTDRAWGUID)        : Result := 'The GUID passed to DirectDrawCreate is not a valid DirectDraw driver identifier.';
    HResult(DDERR_INVALIDMODE)                  : Result := 'DirectDraw does not support the requested mode.';
    HResult(DDERR_INVALIDOBJECT)                : Result := 'DirectDraw received a pointer that was an invalid DIRECTDRAW object.';
    HResult(DDERR_INVALIDPARAMS)                : Result := 'One or more of the parameters passed to the function are incorrect.';
    HResult(DDERR_INVALIDPIXELFORMAT)           : Result := 'The pixel format was invalid as specified.';
    HResult(DDERR_INVALIDPOSITION)              : Result := 'Returned when the position of the overlay on the destination is no longer legal for that destination.';
    HResult(DDERR_INVALIDRECT)                  : Result := 'Rectangle provided was invalid.';
    HResult(DDERR_LOCKEDSURFACES)               : Result := 'Operation could not be carried out because one or more surfaces are locked.';
    HResult(DDERR_NO3D)                         : Result := 'There is no 3D present.';
    HResult(DDERR_NOALPHAHW)                    : Result := 'Operation could not be carried out because there is no alpha accleration hardware present or available.';
    HResult(DDERR_NOBLTHW)                      : Result := 'No blitter hardware present.';
    HResult(DDERR_NOCLIPLIST)                   : Result := 'No cliplist available.';
    HResult(DDERR_NOCLIPPERATTACHED)            : Result := 'No clipper object attached to surface object.';
    HResult(DDERR_NOCOLORCONVHW)                : Result := 'Operation could not be carried out because there is no color conversion hardware present or available.';
    HResult(DDERR_NOCOLORKEY)                   : Result := 'Surface does not currently have a color key';
    HResult(DDERR_NOCOLORKEYHW)                 : Result := 'Operation could not be carried out because there is no hardware support of the destination color key.';
    HResult(DDERR_NOCOOPERATIVELEVELSET)        : Result := 'Create function called without DirectDraw object method SetCooperativeLevel being called.';
    HResult(DDERR_NODC)                         : Result := 'No DC was ever created for this surface.';
    HResult(DDERR_NODDROPSHW)                   : Result := 'No DirectDraw ROP hardware.';
    HResult(DDERR_NODIRECTDRAWHW)               : Result := 'A hardware-only DirectDraw object creation was attempted but the driver did not support any hardware.';
    HResult(DDERR_NOEMULATION)                  : Result := 'Software emulation not available.';
    HResult(DDERR_NOEXCLUSIVEMODE)              : Result := 'Operation requires the application to have exclusive mode but the application does not have exclusive mode.';
    HResult(DDERR_NOFLIPHW)                     : Result := 'Flipping visible surfaces is not supported.';
    HResult(DDERR_NOGDI)                        : Result := 'There is no GDI present.';
    HResult(DDERR_NOHWND)                       : Result := 'Clipper notification requires an HWND or no HWND has previously been set as the CooperativeLevel HWND.';
    HResult(DDERR_NOMIRRORHW)                   : Result := 'Operation could not be carried out because there is no hardware present or available.';
    HResult(DDERR_NOOVERLAYDEST)                : Result := 'Returned when GetOverlayPosition is called on an overlay that UpdateOverlay has never been called on to establish a destination.';
    HResult(DDERR_NOOVERLAYHW)                  : Result := 'Operation could not be carried out because there is no overlay hardware present or available.';
    HResult(DDERR_NOPALETTEATTACHED)            : Result := 'No palette object attached to this surface.';
    HResult(DDERR_NOPALETTEHW)                  : Result := 'No hardware support for 16 or 256 color palettes.';
    HResult(DDERR_NORASTEROPHW)                 : Result := 'Operation could not be carried out because there is no appropriate raster op hardware present or available.';
    HResult(DDERR_NOROTATIONHW)                 : Result := 'Operation could not be carried out because there is no rotation hardware present or available.';
    HResult(DDERR_NOSTRETCHHW)                  : Result := 'Operation could not be carried out because there is no hardware support for stretching.';
    HResult(DDERR_NOT4BITCOLOR)                 : Result := 'DirectDrawSurface is not in 4 bit color palette and the requested operation requires 4 bit color palette.';
    HResult(DDERR_NOT4BITCOLORINDEX)            : Result := 'DirectDrawSurface is not in 4 bit color index palette and the requested operation requires 4 bit color index palette.';
    HResult(DDERR_NOT8BITCOLOR)                 : Result := 'DirectDrawSurface is not in 8 bit color mode and the requested operation requires 8 bit color.';
    HResult(DDERR_NOTAOVERLAYSURFACE)           : Result := 'Returned when an overlay member is called for a non-overlay surface.';
    HResult(DDERR_NOTEXTUREHW)                  : Result := 'Operation could not be carried out because there is no texture mapping hardware present or available.';
    HResult(DDERR_NOTFLIPPABLE)                 : Result := 'An attempt has been made to flip a surface that is not flippable.';
    HResult(DDERR_NOTFOUND)                     : Result := 'Requested item was not found.';
    HResult(DDERR_NOTLOCKED)                    : Result := 'Surface was not locked.  An attempt to unlock a surface that was not locked at all, or by this process, has been attempted.';
    HResult(DDERR_NOTPALETTIZED)                : Result := 'The surface being used is not a palette-based surface.';
    HResult(DDERR_NOVSYNCHW)                    : Result := 'Operation could not be carried out because there is no hardware support for vertical blank synchronized operations.';
    HResult(DDERR_NOZBUFFERHW)                  : Result := 'Operation could not be carried out because there is no hardware support for zbuffer blitting.';
    HResult(DDERR_NOZOVERLAYHW)                 : Result := 'Overlay surfaces could not be z layered based on their BltOrder because the hardware does not support z layering of overlays.';
    HResult(DDERR_OUTOFCAPS)                    : Result := 'The hardware needed for the requested operation has already been allocated.';
    HResult(DDERR_OUTOFMEMORY)                  : Result := 'DirectDraw does not have enough memory to perform the operation.';
    HResult(DDERR_OUTOFVIDEOMEMORY)             : Result := 'DirectDraw does not have enough memory to perform the operation.';
    HResult(DDERR_OVERLAYCANTCLIP)              : Result := 'The hardware does not support clipped overlays.';
    HResult(DDERR_OVERLAYCOLORKEYONLYONEACTIVE) : Result := 'Can only have ony color key active at one time for overlays.';
    HResult(DDERR_OVERLAYNOTVISIBLE)            : Result := 'Returned when GetOverlayPosition is called on a hidden overlay.';
    HResult(DDERR_PALETTEBUSY)                  : Result := 'Access to this palette is being refused because the palette is already locked by another thread.';
    HResult(DDERR_PRIMARYSURFACEALREADYEXISTS)  : Result := 'This process already has created a primary surface.';
    HResult(DDERR_REGIONTOOSMALL)               : Result := 'Region passed to Clipper::GetClipList is too small.';
    HResult(DDERR_SURFACEALREADYATTACHED)       : Result := 'This surface is already attached to the surface it is being attached to.';
    HResult(DDERR_SURFACEALREADYDEPENDENT)      : Result := 'This surface is already a dependency of the surface it is being made a dependency of.';
    HResult(DDERR_SURFACEBUSY)                  : Result := 'Access to this surface is being refused because the surface is already locked by another thread.';
    HResult(DDERR_SURFACEISOBSCURED)            : Result := 'Access to surface refused because the surface is obscured.';
    HResult(DDERR_SURFACELOST)                  : Result := 'Access to this surface is being refused because the surface memory is gone. The DirectDrawSurface object representing this surface should have Restore called on it.';
    HResult(DDERR_SURFACENOTATTACHED)           : Result := 'The requested surface is not attached.';
    HResult(DDERR_TOOBIGHEIGHT)                 : Result := 'Height requested by DirectDraw is too large.';
    HResult(DDERR_TOOBIGSIZE)                   : Result := 'Size requested by DirectDraw is too large, but the individual height and width are OK.';
    HResult(DDERR_TOOBIGWIDTH)                  : Result := 'Width requested by DirectDraw is too large.';
    HResult(DDERR_UNSUPPORTED)                  : Result := 'Action not supported.';
    HResult(DDERR_UNSUPPORTEDFORMAT)            : Result := 'FOURCC format requested is unsupported by DirectDraw.';
    HResult(DDERR_UNSUPPORTEDMASK)              : Result := 'Bitmask in the pixel format requested is unsupported by DirectDraw.';
    HResult(DDERR_VERTICALBLANKINPROGRESS)      : Result := 'Vertical blank is in progress.';
    HResult(DDERR_WASSTILLDRAWING)              : Result := 'Informs DirectDraw that the previous Blt which is transfering information to or from this Surface is incomplete.';
    HResult(DDERR_WRONGMODE)                    : Result := 'This surface can not be restored because it was created in a different mode.';
    HResult(DDERR_XALIGN)                       : Result := 'Rectangle provided was not horizontally aligned on required boundary.';
    // new:
    HResult(DDERR_OVERLAPPINGRECTS)             : Result := 'Operation could not be carried out because the source and destination rectangles are on the same surface and overlap each other.';
    HResult(DDERR_INVALIDSTREAM)                : Result := 'The specified stream contains invalid data';
    HResult(DDERR_UNSUPPORTEDMODE)              : Result := 'The display is currently in an unsupported mode';
    HResult(DDERR_NOMIPMAPHW)                   : Result := 'Operation could not be carried out because there is no mip-map texture mapping hardware present or available.';
    HResult(DDERR_INVALIDSURFACETYPE)           : Result := 'The requested action could not be performed because the surface was of the wrong type.';
    HResult(DDERR_NOOPTIMIZEHW)                 : Result := 'Device does not support optimized surfaces, therefore no video memory optimized surfaces';
    HResult(DDERR_NOTLOADED)                    : Result := 'Surface is an optimized surface, but has not yet been allocated any memory';
    HResult(DDERR_NOFOCUSWINDOW)                : Result := 'Attempt was made to create or set a device window without first setting the focus window';
    HResult(DDERR_DCALREADYCREATED)             : Result := 'A DC has already been returned for this surface. Only one DC can be retrieved per surface.';
    HResult(DDERR_NONONLOCALVIDMEM)             : Result := 'An attempt was made to allocate non-local video memory from a device that does not support non-local video memory.';
    HResult(DDERR_CANTPAGELOCK)                 : Result := 'The attempt to page lock a surface failed.';
    HResult(DDERR_CANTPAGEUNLOCK)               : Result := 'The attempt to page unlock a surface failed.';
    HResult(DDERR_NOTPAGELOCKED)                : Result := 'An attempt was made to page unlock a surface with no outstanding page locks.';
    HResult(DDERR_MOREDATA)                     : Result := 'There is more data available than the specified buffer size could hold';
    HResult(DDERR_EXPIRED)                      : Result := 'The data has expired and is therefore no longer valid.';
    HResult(DDERR_VIDEONOTACTIVE)               : Result := 'The video port is not active';
    HResult(DDERR_DEVICEDOESNTOWNSURFACE)       : Result := 'Surfaces created by one direct draw device cannot be used directly by another direct draw device.';
    HResult(DDERR_NOTINITIALIZED)               : Result := 'An attempt was made to invoke an interface member of a DirectDraw object created by CoCreateInstance() before it was initialized.';
...
initialization
begin
  if (not IsNTandDelphiRunning)  AND (DDrawDLL = 0) then
  begin
    DirektDraw_OK := False;
  ...
  {$ENDIF}
      IF NOT Assigned(DirectDrawEnumerate) THEN          // Martin
        Exit;
