...
TYPE
  ...
  PUBLIC
    ...
    FUNCTION SucheHeader(Dateistream: TDateiStream; StartCodes: TStartCodes; Position: Int64 = -1; Startpos: Boolean = False): Integer; OVERLOAD;
    ...
    FUNCTION Vergleichen(Header: THeader): Integer;  VIRTUAL;
  ...
  PUBLIC
    ...
    FUNCTION Vergleichen(Header: THeader): Integer;  OVERRIDE;
...
// sucht den nächsten Header der in StartCodes vorkommt, Startpos setzt den Lesezeiger an den Anfang des Startcodes
//  Startcode : Ok, Startcode gefunden
// -1..-3 siehe TDateistream.LeseByte
FUNCTION THeader.SucheHeader(Dateistream: TDateiStream; StartCodes: TStartCodes; Position: Int64 = -1; Startpos: Boolean = False): Integer;
...
// -11.. siehe LeseHeader(Dateistream ...) des abgeleitenden Objektes
// -21.. siehe LeseHeader(Dateistream ...) des übergebenen Objektes
...
FUNCTION THeader.Vergleichen(Header: THeader): Integer;
BEGIN
  Result := -1;
END;
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      FBitrate := (Daten[4] SHL 10) + (Daten[5] SHL 2) + ((Daten[6] AND $C0) SHR 6);
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          FBitrate := FBitrate + ((Daten[2] AND $1F) SHL 25) + ((Daten[3] AND $FE) SHL 17);
...
// vergleicht die eigenen Eigenschaften mit denen eines anderen Sequenzheaders
//  1 : Header gleich
//  0 : Header gleich (ohne Bitrate)
// -1 : Header verschieden
// -2 : kein Sequenzheader übergeben
FUNCTION TSequenzHeader.Vergleichen(Header: THeader): Integer;
BEGIN
  IF Assigned(Header) AND (Header IS TSequenzHeader) THEN
    IF (FBildBreite = TSequenzHeader(Header).FBildBreite) AND
       (FBildHoehe = TSequenzHeader(Header).FBildHoehe) AND
       (FSeitenverhaeltnis = TSequenzHeader(Header).FSeitenverhaeltnis) AND
       (FFramerate = TSequenzHeader(Header).FFramerate) AND
       (FVBVPuffer = TSequenzHeader(Header).FVBVPuffer) AND
       (FProfilLevel = TSequenzHeader(Header).FProfilLevel) AND
       (FProgressive = TSequenzHeader(Header).FProgressive) AND
       (FChromaFormat = TSequenzHeader(Header).FChromaFormat) AND
       (FLowDelay = TSequenzHeader(Header).FLowDelay) THEN
      IF FBitrate = TSequenzHeader(Header).FBitrate THEN
        Result := 1
      ELSE
        Result := 0
    ELSE
