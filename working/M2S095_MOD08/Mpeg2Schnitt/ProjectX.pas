...
type
  ...
  public
    ...
    procedure Suche_abbrechen(Sender: TObject);
...
var
  ...
  Abbruch: Boolean;
  ProgressForm: TForm;
  ProgressFormPanel: TPanel;
  ProgressFormCancelButton: TButton;
  ProgressFormLabel: TLabel;
...
procedure TProjectXForm.Suche_abbrechen;
//Dateisuche abbrechen
begin
  ProjectXForm.JvSearchFiles1.Abort;
end;
...
begin
  ...
  CheckBox1.Caption:=ResString(126);
end;
...
begin
  ...
  ProjectXForm.CheckBox1.Checked:=INI.ReadBool('ProjectX','Auto_Erkennung',true);
  INI.Free;
end;
...
begin
  ...
  INI.WriteBool('ProjectX','Auto_Erkennung',ProjectXForm.CheckBox1.Checked);
  INI.Free;
end;
...
begin
  ...
  for Counter:=0 to ListView1.Items.Count-1 do
    begin
      ...
      ProjectXDateienMB:=ProjectXDateienMB+StrToInt(AnsiReplaceStr(ListView1.Items.Item[Counter].SubItems.Strings[2],' MB',''));
      ImportDateien.Add(ListView1.Items.Item[Counter].Caption);
    end;
  Memo1.Clear;

  //FAT Partition Check  
  if (ProjectXDateienMB>4096) then
    begin
      if (Pos('FAT',GetDriveFormat(AnsiLeftStr(ProjectXAusgabeOrdner,3)))>0) then
        begin
          if ImportDateien<>NIL then
            ImportDateien.Free;
          Memo1.SendToBack;
          Kontrollen_freigeben;
          ShowMessage(AnsiReplaceStr(ResString(128),'%1','"'+ProjectXAusgabeOrdner+'"'));
          exit;
        end;
    end;
...
begin
  Compare:=StrToInt(StringReplace(Item2.SubItems[2],' MB','',[rfReplaceAll, rfIgnoreCase])) - StrToInt(StringReplace(Item1.SubItems[2],' MB','',[rfReplaceAll, rfIgnoreCase]));
end;
...
begin
  ...
  //maximal 999 Zeilen in Memo
  if Memo1.Lines.Count>999 then
...
begin
  ...
  AboutMemo.Lines.Add('Version '+MODVersion+' (Build '+MODBuildDate+')');
  try
  AboutMemo.Lines.Add(MediaInfo.MediaInfoDLLVersion);
  except
  AboutMemo.Lines.Add('');
  end;
...
begin
  ...
  try
  //Datei Informationen in ListView speichern
  ListItem:=ListView1.Items.Add;
  ListItem.Caption:=ExtractFileName(OpenDialog1.FileName);
  //Codec auswerten oder Dateiendung nehmen
  Application.ProcessMessages;
  if CheckBox1.Checked=true then
    begin
      if MediaInfo.MediaInfo_DLL_OK=true then
        ListItem.SubItems.Add(MediaInfoFileCodec(OpenDialog1.FileName))
      else
  ...
  if ListItem.SubItems.Strings[0]='' then
    ListItem.SubItems.Strings[0]:=StringReplace(ExtractFileExt(OpenDialog1.FileName),'.','',[rfReplaceAll, rfIgnoreCase]);
  ...
  //sortieren, damit größte Datei (i.d.R. Video) später als erstes in PX geladen wird (wichtig für Audio/Video Sync in PX)
  finally
...
begin
  ...
  //Hinweis Meldung
  if (FileExists(ProjectXPfad)=true) and (FileExists(JavaPfad)=true) then
    ShowMessage(ResString(117));

  //aufräumen
  INI.Free;
end;

function TProjectXForm.SucheDatei(Name: String): String;
//nach Datei suchen
begin
  //Bedienung sperren
  ProjectX.Kontrollen_sperren;
  MpegEdit.Enabled:=false;

  //Fortschrittsanzeige dynamisch erzeugen
  ProgressForm:=TForm.Create(MpegEdit);
  ProgressForm.BorderIcons:=[];
  ProgressForm.FormStyle:=fsStayOnTop;
  ProgressForm.Position:=poMainFormCenter;
  ProgressForm.Caption:=MpegEdit.Caption;
  ProgressForm.BorderStyle:=bsDialog;
  ProgressForm.Width:=600;
  ProgressForm.Height:=110;
  //zusätzliches Panel wird verwendet, um Flackern von TLabel zu mimimieren
  ProgressFormPanel:=TPanel.Create(ProgressForm);
  ProgressFormPanel.Parent:=ProgressForm;
  ProgressFormPanel.Top:=0;
  ProgressFormPanel.Left:=0;
  ProgressFormPanel.Height:=44;
  ProgressFormPanel.Width:=600;
  ProgressFormPanel.DoubleBuffered:=true;
  ProgressFormPanel.Font.Name:='Tahoma';
  ProgressFormPanel.BevelInner:=bvRaised;
  ProgressFormPanel.BevelOuter:=bvLowered;
  ProgressFormLabel:=TLabel.Create(ProgressFormPanel);
  ProgressFormLabel.Parent:=ProgressFormPanel;
  ProgressFormLabel.AutoSize:=false;
  ProgressFormLabel.Width:=584;
  ProgressFormLabel.Height:=24;
  ProgressFormLabel.Left:=6;
  ProgressFormLabel.Top:=13;
  ProgressFormLabel.Visible:=true;
  ProgressFormLabel.Transparent:=false;
  ProgressFormLabel.Font.Name:='Tahoma';
  ProgressFormLabel.Font.Size:=10;
  ProgressFormCancelButton:=TButton.Create(ProgressForm);
  ProgressFormCancelButton.Parent:=ProgressForm;
  ProgressFormCancelButton.Caption:=ResString(30);
  ProgressFormCancelButton.OnClick:=Suche_abbrechen;
  ProgressFormCancelButton.Width:=150;
  ProgressFormCancelButton.Height:=25;
  ProgressFormCancelButton.Top:=ProgressFormPanel.Top+ProgressFormPanel.Height+4;
  ProgressFormCancelButton.Left:=(ProgressForm.Width div 2)-(ProgressFormCancelButton.Width div 2);
  ProgressFormCancelButton.Font.Name:='Tahoma';
  ProgressForm.Show;
  ProgressFormCancelButton.SetFocus;
  ...
  //Dynamisch erzeugte Fortschrittsanzeige löschen
  ProgressForm.Close;
  ProgressFormLabel.Free;
  ProgressFormPanel.Free;
  ProgressFormCancelButton.Free;
  ProgressForm.Free;

  //Bedienung wieder freigeben
  ProjectX.Kontrollen_freigeben;
  MpegEdit.Enabled:=true;
end;

procedure TProjectXForm.JvSearchFiles1FindFile(Sender: TObject; const AName: String);
//Datei im Suchlauf gefunden
begin
  //Systemmeldungen abarbeiten
  Application.ProcessMessages;

  //Abbrechen beim ersten Fund der gesuchten Datei
  //"*.lnk" Links werden ignoriert
  //Ordner "i386" wird ignoriert, da hier inkompatible JAVA.EXE vorhanden
  if (AnsiPos(UpperCase(JvSearchFiles1.FileParams.FileMask),UpperCase(AName))>0) and (AnsiPos('.LNK',UpperCase(AName))=0) and (AnsiPos('I386',UpperCase(AName))=0) then
    JvSearchFiles1.Abort;

  //Fortschrittsanzeige aktualisieren
  if AnsiPos('JAR',UpperCase(JvSearchFiles1.FileParams.FileMask))>0 then
    ProgressFormLabel.Caption:=ResString(37)+AName;
  if AnsiPos('JAVA',UpperCase(JvSearchFiles1.FileParams.FileMask))>0 then
    ProgressFormLabel.Caption:=ResString(38)+AName;
  if AnsiPos('NERO',UpperCase(JvSearchFiles1.FileParams.FileMask))>0 then
    ProgressFormLabel.Caption:=ResString(113)+AName;
end;
