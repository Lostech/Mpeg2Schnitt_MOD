...
// prüft ob eine Dateiendung in einer Dateiendungsmenge vorkommt
FUNCTION Dateiendungpruefen(Endung, Endungen: STRING): Boolean;
...
FUNCTION Nachrichtenfenster(Nachricht, Titel: STRING; Tasten, Icon: Longint; Ton: Boolean = False): Integer;
// modales Nachrichtenfenster ohne Rückmeldung aus der API
PROCEDURE Meldungsfenster(Nachricht: STRING; Titel: STRING = ''; Ton: Boolean = False);
...
// prüft ob eine Dateiendung in einer Dateiendungsmenge (*.mpv;*.m2v) vorkommt
FUNCTION Dateiendungpruefen(Endung, Endungen: STRING): Boolean;
...
BEGIN
  Result := False;
  IF (Endung <> '') AND (Pos('*.', Endungen) = 1) AND (Length(Endungen) > 2) THEN
  BEGIN
    Endung := UpperCase(Endung);
    Endungen := UpperCase(Endungen);
    IF NOT (Endung[1] = '.') THEN
      Endung := '.' + Endung;
    Pos1 := 2;                                                                  // "*" überspringen
    REPEAT
      Pos2 := PosX(';', Endungen, Pos1, False);
      IF Pos2 = 0 THEN
        Pos2 := Length(Endungen) + 1;
      HString := Copy(Endungen, Pos1, Pos2 - Pos1);
      IF Endung = HString THEN
        Result := True;
      Pos1 := Pos2 + 2;                                                         // "*" überspringen
    UNTIL Result OR (Pos2 = Length(Endungen) + 1);
  END;
END;
...
BEGIN
  Result := False;
  IF (Dateiname <> '') AND (Pos('*.', Dateiendungen) = 1) AND (Length(Dateiendungen) > 2) THEN
  BEGIN
    Pos1 := 2;                                                                  // "*" überspringen
    REPEAT
      Pos2 := PosX(';', DateiendungenVideo, Pos1, False);
      IF Pos2 = 0 THEN
        Pos2 := Length(Dateiendungen) + 1;
      Result := FileExists(Dateiname + Copy(Dateiendungen, Pos1, Pos2 - Pos1));
      Pos1 := Pos2 + 2;                                                         // "*" überspringen
    UNTIL Result OR (Pos2 = Length(Dateiendungen) + 1);
  END;
END;
...
BEGIN
  ...
  BEGIN
    IF Dateiendungpruefen(ExtractFileExt(Dateiname), DateiendungenVideo) THEN
  ...
  BEGIN                                                                         // Audiodateinamen prüfen
    IF Dateiendungpruefen(ExtractFileExt(Dateiname), DateiendungenAudio) THEN
      Result := FileExists(Dateiname)
    ELSE
