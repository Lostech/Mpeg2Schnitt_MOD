...
TYPE
  ...
  // Projektlisten
    ...
    ZusammenhaengendeSchnitteberechnenBox: TCheckBox;
  // SchnittTool
...
//    ersterVideoname : STRING;
  // Initialisieren/Beenden
    PPCL : Boolean;                                 // ProgrammPosition CommandLine
...
procedure THauptprogramm.PauseMenuItemClick(Sender: TObject);
begin
  IF laeuft THEN
    PauseMenuItem.Checked := NOT PauseMenuItem.Checked;
  PauseBtn.Down := PauseMenuItem.Checked;
end;
        ...
        ELSE
          ...
          BEGIN
            ProjektGrid.Hint := Meldunglesen(NIL, 'Meldung340', getString(TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, MeineZeile]).IndexDateierstellen, WortJa, WortNein), 'Indexdateien erstellen: $Text1#') + Chr(13) +
                                ...
                                Meldunglesen(NIL, 'Meldung355', getString(TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, MeineZeile]).ZusammenhaengendeSchnitteberechnen, WortJa, WortNein), 'zusammenhängende Schnitte berechnen: $Text1#') + Chr(13) +
...
BEGIN
  ...
  BEGIN
    IF nurAudioBox.Checked THEN
      VideoGroesse := 0
    ELSE
      VideoGroesse := TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, meineZeile]).VideoGroesse;
    CASE Speicherplatzpruefen(ZieldateinameEdit.Text,
                              VideoGroesse,
                              TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, meineZeile]).AudioGroesse) OF
      -1 : Result := Nachrichtenfenster(Wortlesen(NIL, 'Meldung81', 'Auf dem ausgewähltem Laufwerk ist nicht genug Platz zum Erzeugen der Video- und Audiodateien.')
                                        + ' ' + Wortlesen(NIL, 'Meldung87', 'Weitermachen?'),
                                        Wortlesen(NIL, 'Warnung', 'Warnung'), MB_YESNOCANCEL, MB_ICONWARNING);
      -2 : Result := Nachrichtenfenster(Wortlesen(NIL, 'Meldung86', 'Eine Datei größer 4 GByte soll auf ein FAT(32) Laufwerk geschrieben werden.')
                                        + ' ' + Wortlesen(NIL, 'Meldung87', 'Weitermachen?'),
                                        Wortlesen(NIL, 'Warnung', 'Warnung'), MB_YESNOCANCEL, MB_ICONWARNING);
    ELSE
      Result := IDYES;
...
// Kommandozeile
  ...
  TRY
    PPCL := False;
    Erg := Parameterlesen(Projektliste, ProjektInfo, ProgrammPosition, PPCL);
    IF (Erg AND 1) = 0 THEN
...
BEGIN
  ...
  BEGIN
    ArbeitsumgebungObj.SchnittToolProtokollSplitterPos := ProtokollMemo.Height;
    IF ProjektGrid.Height <> 222 THEN
    BEGIN
      EigenschaftenPanel.Height := 222;
    ...
    ZusammenhaengendeSchnitteberechnenBox.Checked := TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, Zeile]).ZusammenhaengendeSchnitteberechnen;
...
BEGIN
  ...
  BEGIN
    ...
    TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, Zeile]).ZusammenhaengendeSchnitteberechnen := ZusammenhaengendeSchnitteberechnenBox.Checked;
...
BEGIN
  ...
  IF ProjektInfoObj.Schnittpunkteeinzelnschneiden THEN
    Result := Trennzeicheneinfuegen(Result) + Wortlesen(Spracheladen, 'Schnittpunkteberechnen', 'Sb');
      ...
      TRY
        ...
        BEGIN
          ...
          BEGIN
            ...
            ProtokollUnit.Protokoll_schreiben(Meldunglesen(NIL, 'Meldung355', getString(TProjektInfo(ProjektGrid.Objects[pOptionenSpalte, Z]).ZusammenhaengendeSchnitteberechnen, WortJa, WortNein), 'zusammenhängende Schnitte berechnen: $Text1#'), 3);
            AnfangszeitDatei := Now;
  ...
  {          test := True;                            // zum Testen
            ...
            BEGIN
              ...
              CASE Erg OF                                                       // bestimmte Fehler auswerten und ins Protokoll schreiben
                -113430, -113470, -113150, -113250, -113350, -113450, -113550, -113650, -113750,
                -123430, -123470, -123150, -123250, -123350, -123450, -123550, -123650, -123750 :
                  HString := Wortlesen(NIL, 'Meldung22', 'Der Anwender hat den Vorgang abgebrochen.');
                -111001, -121001 :
                  HString := Wortlesen(NIL, 'Meldung81', 'Auf dem ausgewähltem Laufwerk ist nicht genug Platz zum Erzeugen der Video- und Audiodateien.');
                -113427, -113467, -113147, -113247, -113347, -113447, -113547, -113647, -113747,
                -123427, -123467, -123147, -123247, -123347, -123447, -123547, -123647, -123747 :
                  HString := Wortlesen(NIL, 'Meldung82', 'Dateischreibfehler. Datei größer 4 GByte auf FAT32 Laufwerk geschrieben?');
                -113428, -113468, -113148, -113248, -113348, -113448, -113548, -113648, -113748,
                 123428, -123468, -123148, -123248, -123348, -123448, -123548, -123648, -123748 :
                  HString := Wortlesen(NIL, 'Meldung83', 'Dateischreibfehler. Festplatte voll?');
                -111002, -121002 :
                  HString := Wortlesen(NIL, 'Meldung86', 'Eine Datei größer 4 GByte soll auf ein FAT(32) Laufwerk geschrieben werden.');
                -113126, -113226, -113326, -113526, -113626, -113726,
                -156000, -150600, -150060, -150006 :                            // Scriptfehler
                  HString := Meldunglesen(NIL, 'Meldung112', Fehlertext, 'Die Datei $Text1# existiert nicht.');
                -112001, -122001, -113043, -123043 :                            // Schnittliste erstellen, Schneiden
                  HString := Meldunglesen(NIL, 'Meldung114', Fehlertext, 'Die Datei $Text1# läßt sich nicht öffnen.');
                -113023, -123023 :                                              // Schneiden
                  HString := Meldunglesen(NIL, 'Meldung132', Fehlertext, 'Die Datei $Text1# konnte nicht erzeugt werden.');
                -110002, -120002 :                                              // Video- Audioschnitt
                  HString := Meldunglesen(NIL, 'Meldung360', Fehlertext, 'Der Zieldateiname $Text1# kommt als Quelldateiname in der Schnittliste vor.');
                -112002 : HString := Wortlesen(NIL, 'Meldung361', 'Es befinden sich unterschiedliche Dateitypen in der Videospur.');
                -122002 : HString := Wortlesen(NIL, 'Meldung362', 'Es befinden sich unterschiedliche Dateitypen in einer Audiospur.');
                -113128, -113228, -113328, -113528, -113628, -113728,
                -158000, -150800, -150080, -150008 :                            // Scriptfehler
                  HString := Meldunglesen(NIL, 'Meldung363', Fehlertext, 'Das aufgerufene Programm $Text1# existiert nicht.');
              END;
              Fehlertext := '';
              ProtokollUnit.Protokoll_schreiben(WortFehlercode + ': ' + IntToStr(Erg), 2);
              IF HString <> '' THEN
                ProtokollUnit.Protokoll_schreiben(HString, 2);
              IF einProjekt THEN
              BEGIN
                nichtBeenden := True;                                           // es ist ein Fehler aufgetreten
                ProjektGrid.Cells[statusSpalte, Z] := WortWarte;
                Meldungsfenster(WortFehlercode + ': ' + IntToStr(Erg) +
                                #10#13 + HString, WortFehler, True);            // Meldungsfenster mit "Ping"
              END
              ELSE
              BEGIN
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      FortschrittDatei.Update;
      aktuelleZeit := Now - AnfangszeitDatei;
  ...
  //   GesamtFortschrittsanzeige
      ...
      Fortschrittgesamt.Update;
      aktuelleZeit := Now - AnfangszeitGesamt;
    ...
    Application.ProcessMessages;
  END;
  WHILE PauseMenuItem.Checked DO
  BEGIN
    Sleep(300);
    Application.ProcessMessages;
  END;
...
begin
  ...
  PauseBtn := TAlphaSpeedBtn.Create(Self).Replace(PauseBtn);
...
begin
  ...
  skin.AddToMapping(POINT(26, 26), 13);
  skin.MappingDone;
end;
...
begin
  ...
  TastenbeschriftungVerbergen(PauseBtn);
...
begin
  ...
  if skin = nil then begin
    ...
    TAlphaSpeedBtn(PauseBtn).Glyph := nil;
    ...
    TAlphaSpeedBtn(PauseBtn).Glyph := skin.GetBitmap(12);
