...
begin
  ...
  ListView1.Columns[1].Caption:=ResString(11);
  ListView1.Columns[2].Caption:=ResString(12);
...
begin
  ...
  for Counter:=0 to ListView1.Items.Count-1 do
    begin
      ProjectXDateien:=ProjectXDateien+' "'+ListView1.Items.Item[Counter].SubItems.Strings[0]+ListView1.Items.Item[Counter].Caption+'"';
...
begin
  Compare := StrToInt(StringReplace(Item2.SubItems[1],' MB','',[rfReplaceAll, rfIgnoreCase])) - StrToInt(StringReplace(Item1.SubItems[1],' MB','',[rfReplaceAll, rfIgnoreCase]));
end;
...
begin
  ...
  if AnsiPos(UpperCase(JvSearchFiles1.FileParams.FileMask),UpperCase(AName))>0 then
